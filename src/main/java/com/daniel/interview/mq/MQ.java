...
public class MQ {
     ...
     ***** 消息队列过期失效
     rabbitmq可以设置过期时间即TTL，如果消息在queue中积压超过一定的时间就会被rabbitmq清理掉，这个数据就没了。
     由此,不会出现数据大量积压在mq里，但是导致大量的数据会直接丢失

     可以采取批量重导，就是大量积压的时候直接丢弃数据，等过了高峰期以后写个临时程序把丢失的数据找补回来


     ***** 消息队列满了怎么搞？
     创建临时消费者快速消费掉所有的消息。过了高峰期再根据临时消费结果找补回来
